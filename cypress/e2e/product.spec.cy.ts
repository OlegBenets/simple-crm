  describe('Product Tests', () => {
    ...
    it('should navigate to Products page and add new product', () => {
        ...
        cy.get('textarea[placeholder="Description"]').click().type('Test test test'); 
    ...
    it('should click on description and edit', () => {
        ...
        cy.get('.detail-list').contains('Apple M1 Chip').siblings().find('mat-icon').contains('delete').click();
        cy.contains('Apple M1 Chip').should('not.exist');

        cy.get('.detail-list').contains('Apple M1 Pro Chip').siblings().find('mat-icon').contains('edit').click();
